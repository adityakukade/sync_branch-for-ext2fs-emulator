...
#define	NUMBER_OF_GROUPS		((TOTAL_DISK_SIZE - MAX_BLOCK_SIZE) / \
					(MAX_BLOCK_GROUP_SIZE))
...
#define BLOCK_NUM_GDT_PER_GROUP		(((32*NUMBER_OF_GROUPS) + \
...
#define BLOCK_NUM_IT_PER_GROUP 		(((128*INODES_PER_GROUP) + \
					(MAX_BLOCK_SIZE-1)) / MAX_BLOCK_SIZE)
#define DATA_BLOCK_BASE			(INODE_TABLE_BASE + \
					BLOCK_NUM_IT_PER_GROUP)

#define GET_INODE_GROUP(x) 		((x) - 1)/( NUMBER_OF_INODES / \
					NUMBER_OF_GROUPS )
#define GET_DATA_GROUP(x)		((x) - 1)/(BLOCKS_PER_GROUP)
#define SQRT(x)  				( (x) * (x)  )
#define TRI_SQRT(x)  			( (x) * (x) * (x) )
#define WHICH_GROUP_BLONG(x) 		( ( (x) - 1)  / \
					( NUMBER_OF_INODES / NUMBER_OF_GROUPS))
...
int 
ext2_remove(EXT2_NODE *parent, EXT2_NODE *rmfile);
int 
...
int 
ext2_create(EXT2_NODE* parent, const char* entryName, EXT2_NODE* retEntry);
int 
ext2_lookup(EXT2_NODE* parent, const char* entryName, EXT2_NODE* retEntry);

int 
fill_super_block(EXT2_SUPER_BLOCK * sb);
int 
fill_descriptor_table(EXT2_GROUP_DESCRIPTOR_TABLE * gd, 
		EXT2_SUPER_BLOCK * sb);

int 
create_root(DISK_OPERATIONS* disk, EXT2_SUPER_BLOCK * sb);

typedef int(*EXT2_NODE_ADD)(void*, EXT2_NODE*);

int 
block_write(DISK_OPERATIONS* this, SECTOR sector, void* data);
int 
block_read(DISK_OPERATIONS* this, SECTOR sector, void* data);
...
int 
init_super_block(DISK_OPERATIONS* disk, EXT2_SUPER_BLOCK* sb, 
		UINT32 group_number);
int 
init_gdt(DISK_OPERATIONS* disk, EXT2_GROUP_DESCRIPTOR_TABLE * gdt, 
		UINT32 group_number);
int 
init_block_bitmap(DISK_OPERATIONS* disk, UINT32 group_number);
int 
init_inode_bitmap(DISK_OPERATIONS* disk, UINT32 group_number);

int 
set_bit(SECTOR number, BYTE* bitmap);
int 
dump_memory(DISK_OPERATIONS* disk, int sector);
int 
validate_sb(void* block);
int 
get_inode(EXT2_FILESYSTEM* fs, UINT32 inode_num, INODE *inodeBuffer);
int 
read_root_block(EXT2_FILESYSTEM* fs, BYTE* block);
int 
...
int 
find_entry_at_block(BYTE* sector, char* formattedName, UINT32 begin, 
		UINT32 last, UINT32* number);
...
int	
set_gd_free_inode_count(EXT2_NODE* NODE,UINT32 free_inode_block,int num);

int 
set_entry( EXT2_FILESYSTEM* fs,EXT2_DIR_ENTRY_LOCATION* location, 
		EXT2_DIR_ENTRY* value );
int 
insert_entry(EXT2_NODE *, EXT2_NODE *, UINT16 );
void
...
int 
get_data_block_at_inode(EXT2_FILESYSTEM *fs, INODE inode, UINT32 number);
int 
