...
extern void 
printf_by_sel(DISK_OPERATIONS* disk, 
		SHELL_FS_OPERATIONS* fsOprs, 
		const SHELL_ENTRY* parent, 
		SHELL_ENTRY* entry, 
		const char* name, 
		int sel, 
		int num);
...
int 
seperate_string(char* buf, 
		char* ptrs[]);
double 
get_percentage(unsigned int number, 
		unsigned int total);
int 
check_conditions(int conditions);
...
int shell_cmd_sync(int argc, char *argv[]);

static COMMAND g_commands[] =
{
	{ "cd",		shell_cmd_cd,		COND_MOUNT	},
	{ "sync",	shell_cmd_sync,		COND_MOUNT	},
...
{
	int	check;
	FILE	*fp	= fopen("storage/diskimg", "r");
	if (fp == NULL)
	{
		fprintf(stderr, "Error in file opening\n");
		exit(EXIT_FAILURE);
	}
	if (disksim_init(NUMBER_OF_SECTORS, MAX_SECTOR_SIZE, &g_disk) 
			< 0)
	{
	...
	if (argc == 2)
	{
		...
		{
			shell_cmd_format(1, NULL);
			shell_cmd_mount(1, NULL);	
			check = fread((((DISK_MEMORY *)g_disk.pdata)->address),
				(NUMBER_OF_SECTORS * MAX_SECTOR_SIZE),
				1, fp);	
			if (check == 1)
			{
				printf("diskimg has been restored\n");
			}
			else
			{
				printf("Error in disk restoration\n");
			}
			fclose(fp);
			do_shell();	
		}
...
void do_shell(void)
{
	...
	while (-1)
	{
		...
		for (i = 0; i < g_commandsCount; i++)
		{
			...
			{
				if (check_conditions(g_commands[i].conditions) 
						== 0)
				{
...
int shell_cmd_cd(int argc, char* argv[])
{
	...
	else
	{
		...
		{
			result = g_fsOprs.lookup(&g_disk, &g_fsOprs, 
					&g_currentDir, &newEntry, argv[1]);
 ...
 */
int 
shell_cmd_exit(int argc, char* argv[])
{
...
/**
 * What happens when you put mount in the shell.
 * It mounts the filesystem.
 */
int 
shell_cmd_mount(int argc, char* argv[])
{
	int result;
...
int shell_cmd_touch(int argc, char* argv[])
{
	...
	result = g_fsOprs.fileOprs->create(&g_disk, &g_fsOprs, &g_currentDir, 
			argv[1], &entry);
...
/**
 * fill.
 * create a new file with some size
 */
int shell_cmd_fill(int argc, char* argv[])
{
	SHELL_ENTRY	entry;
	...
	g_fsOprs.fileOprs->write( &g_disk, &g_fsOprs, &g_currentDir, 
			&entry, 0, size, buffer);
...
int shell_cmd_rm(int argc, char* argv[])
{
	...
	for (i = 1; i < argc; i++)
	{
		if (g_fsOprs.fileOprs->remove(&g_disk, &g_fsOprs, 
					&g_currentDir, argv[i]))
...
/**
 * What happens when you type format in the EXT2 shell.
 * Formats disk for storing data.
 */
int shell_cmd_format(int argc, char* argv[])
{
	int		result;
 ...
 */
int
shell_cmd_sync(int argc, char *argv[])
{
	int	result;

	result = g_fs.sync(&g_disk);

	if (result < 0)
	{
		printf("%s syncing is failed\n", g_fs.name);
		return -1;
	}

	printf("disk has been synced successfully\n");
	return 0;
}
...
int shell_cmd_df(int argc, char* argv[])
{
	...
	printf("free sectors : %u(%.2lf%%)\tused sectors : %u(%.2lf%%)\t"
			"total : %u\n",
			total - used, 
 ...
 */
int 
shell_cmd_mkdir(int argc, char* argv[])
{
	SHELL_ENTRY	entry;
	int result;
	...
	result = g_fsOprs.mkdir(&g_disk, &g_fsOprs, &g_currentDir, argv[1], 
			&entry);
...
int shell_cmd_cat(int argc, char* argv[])
{
	...
	result = g_fsOprs.lookup(&g_disk, &g_fsOprs, &g_currentDir, 
			&entry, argv[1]);
	if (result)
	{
	...
	while ((result = (g_fsOprs.fileOprs->read(&g_disk, &g_fsOprs, 
			&g_currentDir, &entry, offset, 1024, buf))) > 0)
	{
		printf("%s", buf);
		offset += 1024;
...
int shell_cmd_ls(int argc, char* argv[])
{
	...
	{
		printf("|%s|%-3d|%-12d|\n",
			current->entry.name, current->entry.isDirectory, 
			current->entry.size);
